...
class VerificationScreenController extends GetxController {
  ...
  void onResendButtonTap() {
    if (isDurationOver()) {
      ...
      AppDialogs.showErrorDialog(
          messageText: AppLanguageTranslation.waitTransKey.toCurrentLanguage);
    }
  }
      ...
      if (response == null) {
        AppDialogs.showErrorDialog(
            messageText: AppLanguageTranslation
                .noResponseFoundTransKey.toCurrentLanguage);
        return;
  ...
  Future<void> resendCode() async {
    if (isForRegistration) {
      ...
      if (response == null) {
        AppDialogs.showErrorDialog(
            messageText: AppLanguageTranslation
                .waitResendCodeTransKey.toCurrentLanguage);
        return;
      } else if (response.error) {
        AppDialogs.showErrorDialog(messageText: response.msg);
        return;
  ...
  onSuccessSendingOTP(OtpRequestResponse response) {
    AppDialogs.showSuccessDialog(
        messageText:
            AppLanguageTranslation.noResponseCodeTransKey.toCurrentLanguage);
    _resetTimer();
  }
...
*/
  ...
  Future<void> fetchUserDetails(String token) async {
    ...
    getLoggedInOwnerDetails(token);
    // UserDetailsResponse? response = await APIRepo.getUser(token);
  }

  Future<void> getLoggedInOwnerDetails(String token) async {
